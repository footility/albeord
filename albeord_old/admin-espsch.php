...
function elimina_esportate(){
	global $dbh;
	$resP=mysql_query("SELECT id FROM schede WHERE stato=1 AND backup = 1");
	...
	mysql_query("DELETE FROM ordini WHERE backup  = 1");
}
...
if ($_REQUEST["esporta_schede"] && (count($idsSchede) || count($idsOrdini)) && $percorso) {
	...
	if($idsSchede){
	    $resP=mysql_query("SELECT * FROM schede WHERE stato=1 AND (backup = 0 or backup is null) AND id IN (".implode(",",$idsSchede).")");
	...
	if($idsOrdini){
		$resP=mysql_query("SELECT idoperazione, camera,oraoperazione FROM ordini WHERE idoperazione IN (".implode(",",$idsOrdini).") GROUP BY idoperazione");
		
		while($row=mysql_fetch_assoc($resP)){
			$resPt=mysql_query("SELECT * FROM ordini WHERE idoperazione =".$row["idoperazione"]);
		
			while($data=mysql_fetch_assoc($resPt)){
				$final = array();
				$final["ordini"][]=$data;
...
<table width='980' border='0' cellspacing='0' cellpadding='3' align='center' style="border:1px solid #cccccc" class="esp" bgcolor="#FFFFFF">
	...
	while ($scheda = mysql_fetch_assoc($res)){
		...
		$cnt++;
		?>
			<tr class="<?php  if($cnt%2==0) echo "pari" ?> <?php  if($scheda["backup"]) echo "isbackup" ?>">
				<td ><?= $scheda["camera"] ?></td>
				...
				<td>
					<?php  if(!$scheda["backup"]){ ?>
					<input type="checkbox" class="<?php  if($scheda["backup"]) echo "isbackup" ?>" name="schede[<?php echo $scheda["id"] ?>]" <?php if($_REQUEST["schede"][$scheda["id"]]) echo "checked='checked'"; ?> value="<?php echo $scheda["id"] ?>"/>
					<?php } ?>
				</td>
			</tr>
			<?
...
<table class="esp" width='980' border='0' cellspacing='0' cellpadding='3' align='center' style="border:1px solid #cccccc" bgcolor="#FFFFFF">
	...
	while ($scheda = mysql_fetch_assoc($res)){
		...
		<tr  class="<?php  if($cnt%2==0) echo "pari" ?> <?php  if($scheda["backup"]) echo "isbackup" ?>">
			...
			<td>
				<?php  if(!$scheda["backup"]){ ?>
				<input type="checkbox" class="<?php  if($scheda["backup"]) echo "isbackup" ?>" name="ordini[<?php echo $scheda["idoperazione"] ?>]" <?php if($_REQUEST["ordini"][$scheda["idoperazione"]]) echo "checked='checked'"; ?> value="<?php echo $scheda["idoperazione"] ?>"/>
				<?php } ?>
			</td>
		</tr>
