...
function drawtable(pezzi, into){
	...
	for (id in pezzi){
		if (pezzi[id]){
			...
			stringa += "<tr style='cursor:hand' class='carrello_c"+((conter%2==0)?1:2)+"'><td height='40' onclick=\"sottrai('"+id+"')\">"+pezzi[id][2]+"</td><td onclick=\"sottrai('"+id+"')\">"+pezzi[id][1]+"</td><td align='right' onclick=\"sottrai('"+id+"')\">"+show_prezzo(pezzi[id][3]*pezzi[id][2])+" &euro;</td>";
...
//            stringa += "<td bgcolor='#cccc00' style='border-bottom:1px solid #000000' onclick=\"mostra('1')\">&nbsp;&nbsp;+&nbsp;&nbsp;</td>";
	...
	stringa +=  "<tr><td colspan='3' align='center' style='border:1px solid #000000; height:30px;cursor:hand' onclick='concludi_funct()'>Concludi</td></tr>";
	stringa +=  "</tbody></table>";
