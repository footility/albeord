...
$httpdata   = file_get_contents($fileremoto."?secret=".$secret);
...
foreach ($pezzi as $righe){
    foreach (explode('|',$righe) as $stringa){
        ...
        if (preg_match('/\[f[0-9]+\]/',$stringa)){ // usa font
        ...
        if (preg_match('/\[x[0-9]+\]/',$stringa)){ // sposta cord X
