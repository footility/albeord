...
function stampawbs($dbh, $IDORD, $TIPO){
	...
	<script language='VBScript' type='text/vbscript'>
		...
		<?php 'WshShell.Run """'.$PathS.'php.exe"" -n -d extension_dir=""'.$PathS.'"" ""'.$PathS.'stampa.php"" ""'.$dir.'"" ""'.$secret.'"" ",0';	?>
		<?php 'WshShell.Run """'.$PathS.'php.exe"" -n ""'.$PathS.'stampa.php""",0';	?>
	</script>
	<?php
}
function top_menu(){
...
<table style="cursor:pointer" class="topmenu" width='980' border='0' cellspacing='1' cellpadding='3' bgcolor='#000000' align='center'>
	<tr bgcolor='#ffffff'>
		...
		foreach($GlobPrivilegi as $key => $val){
			if ($PRIVILEGI[$key]){
				?><td width='<?= $w ?>' onClick="location.href='<?= $key ?>-core.php'" align='center' <?= (strstr(basename($_SERVER["PHP_SELF"]), "$key-"))?"bgcolor='#cccccc'":" onmouseout=\"this.style.backgroundColor='#ffffff'\" onmouseover=\"this.style.backgroundColor='#cccccc'\" " ?>> <?= $val ?></td><?php
			}
		}
		?>
...
if (strstr($_SERVER['HTTP_USER_AGENT'],"MSIE")===false){
	?><center style="color:#7C0C0C">Albeord &egrave; in grado di stampare unicamente con Internet Explorer 6.0 o superiore</center><?php
}
}
