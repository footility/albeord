...
function disegna_cal($g, $m, $a, $time=0){
	...
	<select name="<?= $g ?>">
		...
		<? for ($d=1; $d<32;$d++) echo "<option value=\"$d\" ".(($d==$g_sel)?"selected":"").">$d</option>\n";	?>
	</select>
	<select name="<?= $m ?>">
		<option value="0">Mese</option>
		<? for ($d=1; $d<=12;$d++) echo "<option value=\"$d\" ".(($d==$m_sel)?"selected":"").">".$MesiTradotti[$d]."</option>\n";?>
	</select>
	<select name="<?= $a ?>">
		<option value="0">Anno</option>
		<? for ($d=date("Y")-2; $d<date("Y")+3;$d++) echo "<option value=\"$d\" ".(($d==$a_sel)?"selected":"").">$d</option>\n";?>
	</select>
	<?
}
...
function stampawbs($dbh, $IDORD, $TIPO){
	...
	<script language='VBScript' type='text/vbscript'>
		...
		<?=  'WshShell.Run "'.$PathS.'php.exe -n -d extension_dir='.$PathS.' '.$PathS.'stampa.php '.$dir.' '.$secret.'", 0'; ?>	
		<? 'WshShell.Run """'.$PathS.'php.exe"" -n -d extension_dir=""'.$PathS.'"" ""'.$PathS.'stampa.php"" ""'.$dir.'"" ""'.$secret.'"" ",0';	?>
		<? 'WshShell.Run """'.$PathS.'php.exe"" -n ""'.$PathS.'stampa.php""",0';	?>
	</script>
	<?
}
function top_menu(){
...
?>
<table style="cursor:pointer" class="topmenu" width='980' border='0' cellspacing='1' cellpadding='3' bgcolor='#000000' align='center'>
	<tr bgcolor='#ffffff'>
		<?
		foreach($GlobPrivilegi as $key => $val){
			if ($PRIVILEGI[$key]){
				?><td width='<?= $w ?>' onClick="location.href='<?= $key ?>-core.php'" align='center' <?= (strstr(basename($_SERVER["PHP_SELF"]), "$key-"))?"bgcolor='#cccccc'":" onmouseout=\"this.style.backgroundColor='#ffffff'\" onmouseover=\"this.style.backgroundColor='#cccccc'\" " ?>> <?= $val ?></td><?
			}
		}
		?>
		<td bgcolor='#EBD1CE' width='<?= $logoutw ?>' onClick="if (confirm('Continuare?'))location.href='index.php?logout=1'" align='center'>Logout</td>
	</tr>
</table>
<?
if (strstr($_SERVER['HTTP_USER_AGENT'],"MSIE")===false){
	?><center style="color:#7C0C0C">Albeord &egrave; in grado di stampare unicamente con Internet Explorer 6.0 o superiore</center><?
}
}
...
function getPrezziScheda($dbh, $ID){
	...
	$resP=mysql_query("SELECT * FROM schede_periodi WHERE idscheda =  $ID order by  dal" );
	while($periodo=mysql_fetch_assoc($resP)){
		// normale
		$prezzi[1]=$periodo["bb"];
		$prezzi[2]=$periodo["hb"];
		$prezzi[3]=$periodo["fb"];
